...
function ArtizanProfiles({ users }) {
	...
	const deleteArtizan = async artizansid => {
		if (window.confirm(`Are you sure you wanna delete this Artizan?`)) {
			const res = await authAxios.delete(`admins/artizans/${artizansid}`)
			await res.data
			if (res.status === 200) {
		...
		return (
			<>
				<StyledTableRow className={classess.data} key={user._id}>
					...
					<StyledTableCell align="right">{user.short_description}</StyledTableCell>
					...
					<StyledTableCell align="right">
						<div onClick={() => deleteArtizan(user.email)} className={classes.delete}>
							{user.isDeleting ? (
								...
								<span>
									<Tooltip id="tooltip-top-start" title="Delete Artizan" placement="top" classes={{ tooltip: classes.tooltip }}>
